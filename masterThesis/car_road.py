...
class Map:
    ...
    def get_points_from_states(self, states):
        ...
        for state in tc:
            ...
            if action == "straight":
                flag = self.go_straight(tc[state]["value"])
            elif action == "left":
                flag = self.turn_left(tc[state]["value"])
            else:
                flag = self.turn_right(tc[state]["value"])
...
if __name__ == "__main__":
    ...
    my_map = Map(200)
    points_ = my_map.get_points_from_states(cases)
    my_map.build_tc(points_)
