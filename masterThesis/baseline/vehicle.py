...
class Car:
    ...
    def __init__(self, speed, steer_ang, map_size):
        ...
        self._str_ang = steer_ang
        self._str_ang_o = steer_ang

        self._x = 0
        self._y = 0
        self._angle = 0

        # Variable containing the distance between the car and the nearest road point
        # When the distance is small (<1) the car goes straight
        # When it's larger, the car turns either left or right
        self._distance = 0
    ...
    def turn_right(self):
        self._str_ang = math.degrees(math.atan(1 / self._speed * 2 * self._distance))
        self._angle = -self._str_ang + self._angle
    ...
    def execute_road(self, nodes):
        ...
        # self._road_x = old_x_vals
        # self._road_y = old_y_vals
        ...
        if (road.is_simple is False) or (is_too_sharp(_interpolate(nodes)) is True):
            ...
            for p, mini_road in enumerate(road_split):
                ...
                if p == 1:
                    self._x = mini_nodes2[0][0]
                    self._y = mini_nodes2[0][1]
                    self._angle = self.get_angle(mini_nodes1[-1], mini_nodes2[0])

                current_pos = [(self._x, self._y)]

                # Keep updating the distance as long as there is still road
...
def min_radius(x, w=5):
    ...
    return mr * 3.280839895  # Min curve
...
def _interpolate(the_test):
    ...
    step_size = 1 / num_nodes
    u_new = arange(0, 1 + step_size, step_size)
    new_x_vals, new_y_vals = splev(u_new, pos_tck)
