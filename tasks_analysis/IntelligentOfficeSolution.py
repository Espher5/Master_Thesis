...
class IntelligentOffice:
    ...
    def manage_light_level(self) -> None:
        ...
        Furthermore, When the last worker leaves the office (i.e., the office is now vacant), the intelligent office
        system stops regulating the light level in the office and then turns off the smart light bulb.
        When the first worker goes back into the office, the system resumes regulating the light level
        """
        if self.is_office_empty():
            self.turn_off_light()
            return
        ...
        self.turn_on_light() if light_level < self.LUX_MIN else self.turn_off_light()
    ...
    def turn_on_exhaust_fan(self):
        if self.fan_switch_on is False:
            GPIO.output(self.FAN_PIN, GPIO.HIGH)
            self.fan_switch_on = True

    def turn_off_exhaust_fan(self):
        if self.fan_switch_on is True:
