...
class IntelligentOffice:
    ...
    DUTY_CYCLE_OPEN_BLINDS = (180 / 18) + 2
    DUTY_CYCLE_CLOSE_BLINDS = (0 / 18) + 2
    ...
    def manage_blinds_based_on_time(self) -> None:
        ...
        if current_time == '08:00:00' and day not in ['SATURDAY', 'SUNDAY']:
            self.open_blinds()
            self.blinds_open = True

        if current_time == '20:00:00' and day not in ['SATURDAY', 'SUNDAY']:
            self.close_blinds()
            self.blinds_open = False

        if current_time.tm_hour in range(8, 19):
            self.blinds_open = True

        if current_time.tm_hour in range(0, 7) and current_time.tm_hour in range(20, 24):
            self.blinds_open = False
    ...
    def open_blinds(self):
        self.change_servo_angle(self.DUTY_CYCLE_OPEN_BLINDS)

    def close_blinds(self):
        self.change_servo_angle(self.DUTY_CYCLE_CLOSE_BLINDS)
    ...
    def manage_light_level(self) -> None:
        ...
        """
        if GPIO.input(self.PHOTO_PIN) < 500:
            self.turn_light_on()
            self.light_on = True

        if GPIO.input(self.PHOTO_PIN) in range(500, 550):
            self.light_on = True

        if GPIO.input(self.PHOTO_PIN) > 550:
            self.turn_light_off()
            self.light_on = False
