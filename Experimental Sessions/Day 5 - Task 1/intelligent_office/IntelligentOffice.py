...
class IntelligentOffice:
    ...
    def manage_blinds_based_on_time(self) -> None:
        ...
        if 8 <= hour <= 20 and self.blinds_open is False:
            duty_cycle = (180 / 18) + 2
    ...
    def is_office_empty(self) -> bool:
        for pin in [self.INFRARED_PIN_1, self.INFRARED_PIN_2, self.INFRARED_PIN_3, self.INFRARED_PIN_4]:
            # Someone is in the office, turn the light on
    ...
    def turn_on_exhaust_fan(self):
        if self.vent_switch_on is False:
            GPIO.output(self.FAN_PIN, GPIO.HIGH)
            self.vent_switch_on = True

    def turn_off_exhaust_fan(self):
        if self.vent_switch_on is True:
