...
class ParkingGarage:
    ...
    def calculate_parking_fee(self, entry_time: str) -> float:
        ...
        exit_time = time.strptime(self.rtc.get_current_time_string(), '%H:%M:%S')
        day = self.rtc.get_current_day()

        if entry_time.tm_hour > exit_time.tm_hour:
            raise ParkingGarageError

        hours = exit_time.tm_hour - entry_time.tm_hour
        minutes = exit_time.tm_min - entry_time.tm_min
    ...
    def turn_light_on(self) -> None:
        ...
        """
        if self.light_on is False and self.get_occupied_spots() > 0:
    ...
    def turn_light_off(self) -> None:
        ...
        """
        if self.light_on is True and self.get_occupied_spots() == 0:
