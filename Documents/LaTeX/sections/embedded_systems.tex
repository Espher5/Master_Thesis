...
Zigbee is a wireless communication protocol that is designed for low-power, low-data-rate applications \cite{Zigbee}; it is often used in sensors and other devices that need to communicate over short distances, such as in home automation systems or industrial control systems. One of its key advantages is the low power consumption, which makes it well-suited for use in devices that need to operate for long periods of time without access to a power source.
